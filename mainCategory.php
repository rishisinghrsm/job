...
<div class="container-fluid mt-5 ">
    <div class="row">
        <div class="col-md-9 m-auto">
            <div class="card  ">
                <div class="card-header trirev green text-white sticky-top ">
                
                    <?php 
                    $id  = $_GET['main'];
                    $cat = "SELECT * FROM firstCategory where id = $id";
                    $catResult = mysqli_query($conn,$cat) or die("Failed To Fatch");
                    $data = mysqli_fetch_assoc($catResult);
                    ?>
                    <h2 class="text-white text-uppercase fw-bold  m-0"><?php echo $data['firstCategory'] ?></h2>
                    <!--  -->
                </div>
...
<?php include 'config.php' ?>
                    <div class="container  ">
                        <?php
                        $num = 1;
                        $sql1 = "SELECT * FROM `notification` WHERE status = 1 AND ( catId = $id )ORDER BY postDate DESC ";
